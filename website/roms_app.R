...
# Shiny app to explore ROMS predictions and covariates by date
...
# Folders
pred_folder <- "model_prediction/ROMS/rasters"
env_folder <- "data_processing/ROMS/rasters"
...
env_rasters <- get_file_dates(folder = env_folder,
                              ...
                              strip = TRUE) |> 
  filter(!str_detect(name, "lunar"))  #remove unneeded variables
...
covar_pal_df <- data.frame(covar = unique(env_rasters$layer),
                           pals = c("tempo","deep","curl","dense","delta","matter","thermal","amp",
