...
navset_card_tab(
  ...
  nav_panel(title = "Yesterday", img(src = "https://github.com/joshcullen/CEG_operationalization/blob/main/model_prediction/ROMS/img/leatherbackTurtle_2024-11-08.png?raw=true", width = "50%")),
...
eke.pal <- colorNumeric(cmocean::cmocean("tempo")(256), domain = values(covars[[1]]), na.color = "transparent")
bv.pal <- colorNumeric(cmocean::cmocean("deep")(256), domain = values(covars[[2]]), na.color = "transparent")
curl.pal <- colorNumeric(cmocean::cmocean("curl")(256), domain = values(covars[[3]]), na.color = "transparent")
ild.pal <- colorNumeric(cmocean::cmocean("dense")(256), domain = values(covars[[4]]), na.color = "transparent")
ssh.pal <- colorNumeric(cmocean::cmocean("delta")(256), domain = values(covars[[5]]), na.color = "transparent")
ssh_sd.pal <- colorNumeric(cmocean::cmocean("matter")(256), domain = values(covars[[6]]), na.color = "transparent")
  ...
  nav_panel(title = "Environ. Predictors",
            card(full_screen = TRUE,
                 leaflet() |> 
                   ...
                   addLayersControl(position = "topright",
                                    baseGroups = c("CartoDB","Satellite","Bathymetry"),
                                    overlayGroups = c("EKE","BV","Curl","ILD","SSH","SSH_SD","SST","SST_SD"),
                                    options = layersControlOptions(collapsed = TRUE, autoZIndex = FALSE)) |> 
                   
                   addRasterImage(covars[[1]], colors = eke.pal, opacity = 1, group = "EKE", layerId = "EKE") |> 
                   addImageQuery(covars[[1]], layerId = "EKE") |>  #add raster  query
                   addLegend(title = "EKE", position = "bottomright", pal = eke.pal, values = values(covars[[1]]),
                             group = "EKE") |> 
